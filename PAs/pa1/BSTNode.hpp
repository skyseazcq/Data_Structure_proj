...
public:
  ...
  BSTNode<Data>* successor() {
	//jason 
	BSTNode<Data>* nextPtr;
	BSTNode<Data>* currPtr;

	if(right != nullptr){
		nextPtr = right;
		while (nextPtr->left != nullptr) nextPtr = nextPtr->left;
	 	return nextPtr;
	}
	else if (this->parent == nullptr) {
		return nullptr; // this is the root
	}
	else{
		nextPtr = this->parent;
		currPtr = this;
		while (nextPtr->parent != nullptr){
			if (nextPtr->left == currPtr) return nextPtr;
			currPtr = nextPtr;
